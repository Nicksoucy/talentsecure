        ...
        <Stack spacing={1.25}>
          ...
          >
            <Typography variant="caption" color="text.secondary">
              {dropPoint
                ? 'Point déposé — ajustez le rayon puis trouvez les CV proches.'
          ...
          {dropPoint && (
            <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
              {onNearbySelect && (
                ...
                >
                  Trouver les CV proches
                  {countLoading ? ' …' : nearCount != null ? ` (${nearCount})` : ''}
                </Button>
              )}
              {!onNearbySelect && nearCount != null && (
                <Chip size="small" color="primary" label={`${nearCount} CV dans ${radiusKm} km`} />
              )}
