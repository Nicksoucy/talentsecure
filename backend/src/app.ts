...
/**
 * Construit et configure l'application Express SANS la démarrer (`app.listen`).
 *
 * Extrait de `server.ts` pour permettre aux tests d'importer une app réelle et
 * complète (toutes les routes, le vrai sanitize/passport/error handler) via
 * Supertest, sans ouvrir de port ni lancer le scheduler. `server.ts` reste le
 * point d'entrée (il appelle `createApp()` puis `listen`).
 *
 * IMPORTANT : l'ordre des middlewares est identique à l'ancien `server.ts`
 * (helmet → cors → rate-limit → logger → stripe-webhook(raw) → json → sanitize →
 * passport → static → health → routes → 404 → error handler). Toute divergence
 * d'ordre fausserait les tests.
 */
export function createApp(): Express {
  ...
  // Sentry capte uniquement les 5xx (le bruit 4xx ne noie pas les alertes).
