...
export async function processVideoUpload(localFilePath: string, originalFilename: string): Promise<string> {
  ...
  // Priority: Cloudflare R2 > Google Drive > Google Cloud Storage > Local
  if (useR2) {
    try {
      console.log('Uploading video to Cloudflare R2...');

      // Upload to R2
      const r2Response = await uploadVideoToR2(localFilePath, fileName);
      ...
      console.log(`Video uploaded to R2. Key: ${r2Response.key}`);

      // Return the R2 key (we'll use this to generate URLs later)
  ...
  // Return the storage path (filename for GCS/Local, fileId for Drive, key for R2)
  return fileName;
}
...
export async function deleteVideo(videoStoragePath: string): Promise<void> {
  try {
    if (useR2) {
      // videoStoragePath is the R2 key
      await deleteVideoFromR2(videoStoragePath);
      console.log(`Video deleted from R2. Key: ${videoStoragePath}`);
    } else if (useGoogleDrive) {
...
export function getVideoUrl(videoStoragePath: string): string {
  if (useR2) {
    // For R2, return public URL (if custom domain) or the key (for signed URL generation)
    return getPublicUrl(videoStoragePath);
  } else if (useGoogleDrive) {
...
/**
 * Get signed URL for R2 video (when not using custom domain)
 * Call this in the controller when serving the video
 */
export async function getR2SignedUrl(videoKey: string, expiresIn: number = 3600): Promise<string> {
  if (!useR2) {
    throw new Error('R2 is not enabled');
  }
  return await getSignedVideoUrl(videoKey, expiresIn);
}
